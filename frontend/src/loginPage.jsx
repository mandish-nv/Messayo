...
export default function Login({ value }) {
    ...
    return (
        <div className="login">
            ...
            <video autoPlay loop muted className={`dokoto ${val === 2 ? 'move-right' : val === 3 ? 'center' : ''}`}>
                <source src="/video.mp4" type="video/mp4" />
            </video>
            <div style={{ display: error == '6' ? 'grid' : 'none',position:'fixed',zIndex:'5',left:'37.3vw',top:'10vh',justifyContent:'center',gap:'1rem' }}>
                <div className='register-successful'>Registered Successfully</div>
                <div align='center'>
                    <Link to={'/login'} className='login-button' onClick={() => { setVal(1); setError(0) }}>Login</Link>
                </div>
            </div>
            <div style={{ display: error == '7' ? 'grid' : 'none',position:'fixed',zIndex:'5',left:'40vw',top:'10vh',justifyContent:'center',gap:'1rem' }}>
                <div className='register-successful'>Login Successfully</div>
            </div>
